...
void insertFirst(int data) //insert link at the first location
{
...
void insertOne(int pos, int data) //insert link at a certain location
{
    int i;
    struct node *link = (struct node *)malloc(sizeof(struct node)); //create a link
    link->data = data;
    ...
    { //itarate through list
        ...
        {
            printf("[x] Error! Position Must be between 1 to %d", length());
    ...
    {
        link->next = current;
        previous->next = link; //bypass the current link
    }
}

void insertEnd(int data) //insert link at the first location
{
    struct node *link = (struct node *)malloc(sizeof(struct node)); //create a link
    link->data = data;
    link->next = head;
...
{
    node *cur = head; //save reference to first link
    node *prev = NULL;
    ...
    {
        while (cur->next != head) //Else traverse till the last node
        {
            prev = cur;
            cur = cur->next; //move to next link
        }
        head = tempNode->next->next; //mark next to first node as first
...
node *delete_from_pos(node *head, int pos)
{
    ...
    if (current == head) //found a match, update the link
    {
        while (previous->next != head) //Else traverse till the last node
            previous = previous->next; //move to next link
